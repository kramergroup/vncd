...
/*
  ...
  most system with X11 and x11vnc installed, but it might not do much apart from
	showing an empty screen with a X as cursor (depending on how xinit is configured).
*/

// FallbackVncConfiguration is a structure holding configurations for the local environment
type FallbackVncConfiguration struct {
...
type FallbackVncSession struct {
	Config      FallbackVncConfiguration // The configuration of the
...
func NewFallbackVncConfiguration() FallbackVncConfiguration {

	return FallbackVncConfiguration{
